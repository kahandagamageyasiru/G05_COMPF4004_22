...
class SubscriptionPage extends StatelessWidget {
  ...
  return InkWell(
    onTap: () async {
      await _makePayment(amount: amount,label:label);
    },
    child: Center(
      child: Container(
        decoration: BoxDecoration(
...
Future<void> _makePayment({required String amount,required String label}) async {
  await PaymentController().makePayment(amount: amount, currency: 'USD', label: label);
  }
}
